use axum::{
    body::Body,
    extract::{Multipart, State, Request, FromRequest, Json},
...
pub async fn remove_bg(
    ...
    if content_type.starts_with("application/json") {
        ...
        let res = match client.post(modal_url)
            .json(&payload)
            .send()
            .await {
                Ok(res) => res,
                Err(e) => {
                    ...
                    return (StatusCode::INTERNAL_SERVER_ERROR, "Failed to connect to processing worker").into_response();
                }
            };
        ...
        let mut multipart = match Multipart::from_request(request, &config).await {
            Ok(m) => m,
            Err(e) => return (StatusCode::BAD_REQUEST, format!("Invalid multipart request: {}", e)).into_response(),
        };
        ...
        let image_bytes = match image_data {
            Some(data) => data,
            None => return (StatusCode::BAD_REQUEST, "No image found in 'image' field").into_response(),
        };
        ...
        (StatusCode::UNSUPPORTED_MEDIA_TYPE, "Content-Type must be application/json or multipart/form-data").into_response()
    }
}
...
async fn handle_modal_response(res: reqwest::Response) -> Response {
    if !res.status().is_success() {
         ...
         return (StatusCode::BAD_GATEWAY, format!("Processing worker returned an error: {}", error_text)).into_response();
    }
